...
void RecursiveCTENode::Serialize(Serializer &serializer) const {
	...
	serializer.WritePropertyWithDefault<vector<unique_ptr<QueryNode>>>(205, "trampolines", trampolines);
}
...
unique_ptr<QueryNode> RecursiveCTENode::Deserialize(Deserializer &deserializer) {
	...
	deserializer.ReadPropertyWithDefault<vector<unique_ptr<QueryNode>>>(205, "trampolines", result->trampolines);
	return std::move(result);
}
