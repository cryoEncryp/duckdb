...
public:
	LogicalRecursiveCTE(string ctename_p, idx_t table_index, idx_t union_index, idx_t column_count, bool union_all,
	    ...
	    : LogicalOperator(LogicalOperatorType::LOGICAL_RECURSIVE_CTE), union_all(union_all),
	      ctename(std::move(ctename_p)), table_index(table_index), union_index(union_index), column_count(column_count) {
