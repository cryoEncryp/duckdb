...
PhysicalOperator &PhysicalPlanGenerator::CreatePlan(LogicalRecursiveCTE &op) {
	...
	if (op.key_targets.empty()) {
		...
		auto &cte = Make<PhysicalRecursiveCTE>(op.ctename, op.table_index, op.types, op.union_all, left,
		...
		for (idx_t i = 0; i < payload_aggregates.size(); i++) {
			auto &dat = payload_aggregates[i];
			payload_aggregates_ptr.push_back(dat.get());
		}

		// Create hashtable that is shared between recursive operator and scan operator
		auto ht = make_shared_ptr<GroupedAggregateHashTable>(context, BufferAllocator::Get(context), distinct_types,
															 payload_types, std::move(payload_aggregates_ptr));

		// share between scans and recurisve cte
		recurring_cte_tables[op.table_index] = ht;
		...
		// BTODO: remove duplication of types and idx
		...
		// BTODO: not needed anymore (?)
		cast_cte.ref_recurring = op.ref_recurring;
		cast_cte.working_table = working_table;

		ht->distinct_types = distinct_types;
		ht->payload_types = payload_types;
		ht->distinct_idx = distinct_idx;
		ht->payload_idx = payload_idx;
...
PhysicalOperator &PhysicalPlanGenerator::CreatePlan(LogicalCTERef &op) {
	...
	if (op.is_recurring) {
		...
		auto rec_cte = recurring_cte_tables.find(op.cte_index);
		if (rec_cte == recurring_cte_tables.end()) {
			throw InvalidInputException("RECURRING can only be used with USING KEY in recursive CTE.");
		}
		auto &chunk_scan = Make<PhysicalHashTableScan>(cte->second.get()->Types(),
		                                                  PhysicalOperatorType::RECURSIVE_RECURRING_CTE_SCAN,
		                                                  op.estimated_cardinality, rec_cte->second);

		return chunk_scan;
	} else {
		auto &chunk_scan = Make<PhysicalColumnDataScan>(
